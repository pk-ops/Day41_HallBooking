...
app.listen(PORT,() => {
  ...
  app.get('/', function (req, res) {
    res.send('Welcome to the Hall booking application')
  })

  //Create Room for hall booking
  app.post("/createroom", async function (req, res) {
    const {seat_count,amenities,room_name,price_per_hour}=req.body;
    const data = req.body;
    if((!seat_count)||(!amenities)||(!room_name)||(!price_per_hour)){
      res.status(400).send({msg:"Insuffecient details, Kindly provide all the following details seat_count, amenities, room_name,price_per_hour"});
    }else{
